...
class ProviderInlineTestCase(TestCase):
    ...
    def test_readonly_fields(self):
        self.assertEqual(
            self.provider_inline.readonly_fields, ("sitter_score", "rating_score", "overall_rank")
        )

    def test_provider_methods(self):
        self.assertEqual(
            self.provider_inline.sitter_score(self.provider), self.provider.search_score.sitter_score
        )
        self.assertEqual(
            self.provider_inline.rating_score(self.provider), self.provider.search_score.rating_score
        )
        self.assertEqual(
            self.provider_inline.overall_rank(self.provider), self.provider.search_score.overall_rank
        )
...
class PersonAdminTestCase(TestCase):
    def setUp(self):
        ...
        self.inline_instances = self.person_admin.get_inline_instances(self.request, self.person)
    ...
    def test_inline_instances_when_person_is_provider(self):
        ...
        self.inline_instances = self.person_admin.get_inline_instances(self.request, self.provider)
